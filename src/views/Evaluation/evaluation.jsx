...
$(document).ready(() => {
  $("body").on("click", "#TblEvaluation #btEdit", function (e) {
    ShowPopUp({
      titleName: "Editar Evaluación",
      htmlBody: EditEvaluation(e),
  ...
  $("#sp_AddEvaluation").click(() => {
    ShowPopUp({
      titleName: "Añadir Evaluación",
      htmlBody: AddEvaluation(),
  ...
  const OnClickSaveEdit = () => {
    let param = {
      ...
      EvaluationHierarchyId:
        $("#dropHierarchy").val() === "0"
          ? null
          : parseInt($("#dropHierarchy").val()),
      PositionMustFill:
        $("#dropPositionMustFill").val() === "0"
          ? null
          : $("#dropPositionMustFill").val(),
    ...
    API.getData("/Evaluations/GetEvaluationDetails")
      .then((res) => {
        ...
        if (res.status === 200) {
          ...
          let calendarBtn =
            "<a href='#' id='btCalendar'  class='fa fa-calendar custom-color size-effect-x2' title='Calendario Evaluación' ></a>&nbsp;";
          let PossibleAswBtn =
            "<a href='#' id='btPossibleAsw'  class='fa fa-reply custom-color size-effect-x2' title='Posibles Respuestas' ></a>&nbsp;";
          let AswBtn =
            "<a href='#' id='btAsw'  class='fa fa-question custom-color size-effect-x2' title='Preguntas' ></a>&nbsp;";
          let EditBtn =
            "<a href='#' id='btEdit'  class='fa fa-pencil-square-o custom-color size-effect-x2' title='Editar Evaluación' ></a>";
          setEvaluations(res.data);

          if (res.data.length > 0) {
            JSON.parse(res.data[0].evaluationHierarchy).forEach((item) => {
              evaluationHierarchyData.push({
                EvaluationHierarchyId: item.EvaluationHierarchyId,
                EvaluationHierarchyName: item.EvaluationHierarchyName,
              });
            });
          }
          res.data.forEach((item) => {
            ...
            if (evaluationHierarchyData.length > 0) {
              evaluationHierarchyData.forEach((element) => {
                if (
                  element.EvaluationHierarchyId === item.evaluationHierarchyId
                ) {
            ...
            dataResult.push({
              ...
              EvaluationObjectName:
                '<span class="capitalized defaultText" data-object-id="' +
                item.evaluationObjectId +
                '">' +
                item.evaluationObject.evaluationObjectName +
                "</span>",
              EvaluationHierarchyName:
                '<span class="capitalized defaultText" data-hierarchy-id="' +
              ...
              PositionMustFill:
                '<span class="capitalized defaultText">' +
                (item.positionMustFill === null
                  ? ""
                  : item.positionMustFill.toLowerCase()) +
                "</span>",
              FillAfterLogin:
                '<span class="capitalized defaultText">' +
                (item.fillAfterLogin !== "N" ? "Si" : "No") +
                "</span>",
              AfterHiringDate:
                '<span class="capitalized defaultText">' +
                (item.afterHiringDate !== "N" ? "Si" : "No") +
                "</span>",
              Inactive:
                '<span class="capitalized defaultText">' +
          ...
          $("#TblEvaluation").DataTable({
            ...
            columns: [
              ...
              {
                data: "EvaluationObjectName",
                title: "Objeto\u00a0Evaluado",
                width: "20%",
                className: "capitalized",
              },
              {
                data: "EvaluationHierarchyName",
                title: "Jerarquía\u00a0Evaluación",
                width: "15%",
                className: "capitalized",
              },
              {
                data: "PositionMustFill",
                title: "Debe\u00a0Llenar",
                width: "15%",
                className: "capitalized",
              },
              {
                data: "FillAfterLogin",
                title: "Llenar\u00a0Inicio\u00a0Sesión",
                width: "10%",
                className: "capitalized",
              },
              {
                data: "AfterHiringDate",
                title: "Llenar\u00a0Luego\u00a0Aniver.",
                width: "10%",
                className: "capitalized",
              },
              {
        ...
        <div className=" htmlPayroll    container">
          <div className="row">
            <div className="lowcolor col-12">
              ...
              <span className="btn btn-success btn-sm" id="sp_AddEvaluation">
