...
$(document).ready(()=>{
    $('body').on('click', '#TblBackendUser #btEdit', function(e){
         ...
         ShowPopUp({titleName: "EDITAR USUARIO", htmlBody: EditBackendUsers(e), handlerEvent: OnClickSaveEdit, TextOk:"Guardar", isDisabled:true });               
              setTimeout(() => {
                      atob($(e.currentTarget).parent().attr('data-permission-text')).split('|').forEach(item => {
    ...
    $('#sp_AddUser').click(()=>{
       ShowPopUp({titleName: "CREAR USUARIO", htmlBody: AddBackendUsers(), handlerEvent: OnClickSaveAdd, TextOk:"Guardar", isDisabled:true, EnabledDisabled:true});
              setTimeout(() => {
                    const dataElement = $('#dropPermission.chosen-select').find('option');
    ...
    $('body').on('click', '#TblBackendUser #btDel', function(e){
        let param = JSON.parse(atob($(e.currentTarget).parent().attr('data-item')))[0];
...
const SaveAddChanges = (params)=>{ 
    API.postData("BackendUser/AddBackendUserAndRole?data=" + JSON.stringify({
...
export default function BackendUser(props) {
            ...
            if (res.status === 200) { 
                ...
                if(res.data.length > 0){
                    ...
                    JSON.parse(res.data[0].backendPermission).forEach(item => {
                        backendPermissionData.push({PermisionId: item.PermisionId, OptionMenuId: item.OptionMenuId, UserId: item.UserId });
                    });
                    JSON.parse(res.data[0].backendCompany).forEach(item => {
                        companyData.push({CompanyId: item.CompanyId, Name: item.Name, CompanyLogo: item.Logo});
                    });
                    JSON.parse(res.data[0].backendOptionMenu).forEach(item => {
                        backendOpMenuData.push({OptionMenuId: item.OptionMenuId, Name: item.Name, IsActive: item.IsActive});
                    }); 
                    JSON.parse(res.data[0].backendUser).forEach(item => {
                        ...
                        backendUserData.push({Alias: item.Alias, 
                                              UserId: item.UserId, 
                                              User: item.User, 
                                              Psw: item.Password, 
                ...
                backendUserData.forEach(item => {
                    dataResult.push({ 
                        Alias: '<span class="capitalized defaultText">' + item.Alias + '</>',
                        User: '<span class="capitalized defaultText" data-userid="'+ item.UserId +'">' + item.User + '</span>',
                        Psw: '<span class="defaultText" title="'+ item.Psw +'">' + item.Psw.substr(0,10) + '...' + '</span>',
                ...
                $("#TblBackendUser").DataTable({
                            ...
                            columns: [  
                                ...
                                { data: "Psw", title: "Contraseña", width:'20%', className:"capitalized" },
 ...
 const GetOptionMenuAndPermission = (dataItem)=>{ 
    ...
    JSON.parse(dataItem.backendOptionMenu).forEach(element => {
        if(element.IsActive === "N" && typeof(backendOpMenuPermissionData.find(c=>c.OptionMenuId === element.OptionMenuId)) === "undefined"){
            backendOpMenuPermissionData.push({ OptionMenuId: element.OptionMenuId, Name: element.Name});
        }  
    });
    $("#TblBackendUser").attr('data-OpcMenuPermission', btoa(JSON.stringify(backendOpMenuPermissionData)));    
 }    
        ...
        <div className=" htmlPayroll    container">
          <div className="row">
            <div className="lowcolor col-12">  
              ...
              <span className="btn btn-success btn-sm" id="sp_AddUser"><i className="fa fa-plus-circle"></i>&nbsp;Crear Usuarios</span>
                        ...
                        <div className="">
                            {!dataLoading ?
                                <div className="scroll-table bordered" Style="min-height:600px">
                                    <table id="TblBackendUser"  className="table table-striped table-bordered display" Style="width:100% !important"></table>
                                </div>                 
                            :
                            <Loading />
                            }
